...
class ResetAllSchoolsSeeder extends Seeder
{
    public function run(): void
    {
        $this->command->info('=== Resetting ALL school data to exact 432 schools ===');

        // ── Step 1: Fix UC sector assignments ─────────────────
        $niloreId = Sector::where('code', 'NILORE')->value('id');

        // UC-52/53/54 were created in previous session with wrong sector (Urban-I)
        UnionCouncil::whereIn('code', ['UC-52', 'UC-53', 'UC-54'])
            ->update(['sector_id' => $niloreId]);

        // Rename UC-53 from "Shakrial" to "New Shakrial"
        UnionCouncil::where('code', 'UC-53')
            ->update(['name' => 'UC-53 New Shakrial']);

        // Create UC-55 Khana Dak II RWP for Nilore
        UnionCouncil::firstOrCreate(
            ['code' => 'UC-55'],
            ['name' => 'UC-55 Khana Dak II RWP', 'sector_id' => $niloreId, 'is_active' => true]
        );

        $this->command->info('UC sector assignments fixed. UC-55 created.');

        // ── Step 2: Nullify user institution_ids ──────────────
        DB::table('users')->whereNotNull('institution_id')->update(['institution_id' => null]);
        $this->command->info('User institution_ids nullified.');

        // ── Step 3: Clear related tables ──────────────────────
        DB::statement('SET FOREIGN_KEY_CHECKS=0');
        DB::table('institution_sections')->truncate();
        DB::table('institution_classes')->truncate();
        DB::table('daily_admissions')->truncate();
        ...
        $this->command->info('All institution-related tables cleared.');

        // ── Step 4: Cache lookups ─────────────────────────────
        $sectors = Sector::pluck('id', 'code')->toArray();
        $ucs     = UnionCouncil::pluck('id', 'code')->toArray();

        // ── Step 5: Insert all 432 schools ────────────────────
        // Format: [emis, name, gender, type, sector_code, uc_code]
        $schools = [
            // ═══════════════════════════════════════════════════
            // URBAN-I (105 schools, EMIS 201-317)
            // ═══════════════════════════════════════════════════
            ...
            // Urban-I continued (EMIS 295-317)
            ...
            // ═══════════════════════════════════════════════════
            // URBAN-II (75 schools, EMIS 250-314)
            // ═══════════════════════════════════════════════════
            ...
            [422, 'IMSB (I-V), CHAHANN MASTAL (F.A) H-11', 'boys', 'I-V', 'TARNOL', 'UC-43'],
            ...
            // ═══════════════════════════════════════════════════
            // SIHALA (76 schools, EMIS 501-575)
            // ═══════════════════════════════════════════════════
            ...
            // ═══════════════════════════════════════════════════
            // TARNOL (64 schools, EMIS 601-655)
            // ═══════════════════════════════════════════════════
            [601, 'IMCB (VI-XII) Tarnol', 'boys', 'VI-XII', 'TARNOL', 'UC-47'],
            [602, 'IMSB (VI-X) Sang Jani', 'boys', 'VI-X', 'TARNOL', 'UC-45'],
            [603, 'IMSB (I-X) Naugazi', 'boys', 'I-X', 'TARNOL', 'UC-43'],
            [604, 'IMSB (I-X) I-14', 'boys', 'I-X', 'TARNOL', 'UC-44'],
            [605, 'IMSB (VI-X) Noon', 'boys', 'VI-X', 'TARNOL', 'UC-45'],
            [606, 'IMSB (I-X) Maira Akku', 'boys', 'I-X', 'TARNOL', 'UC-45'],
            [607, 'IMSB (VI-X) Shah Allah Ditta', 'boys', 'VI-X', 'TARNOL', 'UC-49'],
            [608, 'IMSB (VI-X) Golra', 'boys', 'VI-X', 'TARNOL', 'UC-50'],
            [609, 'IMSB (I-X) Badana Kalan', 'boys', 'I-X', 'TARNOL', 'UC-46'],
            [610, 'IMSG (I-X) Sangjani', 'girls', 'I-X', 'TARNOL', 'UC-45'],
            [611, 'IMSG (I-X) Jhangi Syedan (F.A.)', 'girls', 'I-X', 'TARNOL', 'UC-45'],
            [612, 'IMCG (I-XII) Shah Allah Ditta', 'girls', 'I-XII', 'TARNOL', 'UC-49'],
            [613, 'IMCG (I-XII) Badana Kalan', 'girls', 'I-XII', 'TARNOL', 'UC-46'],
            [614, 'IMCG (I-XII) Tarnaul', 'girls', 'I-XII', 'TARNOL', 'UC-47'],
            [615, 'IMCG (I-XII) Golra', 'girls', 'I-XII', 'TARNOL', 'UC-50'],
            [616, 'IMSG (VI-X) I-14/3', 'girls', 'VI-X', 'TARNOL', 'UC-44'],
            [617, 'IMSG (I-X) Naugazi (F.A)', 'girls', 'I-X', 'TARNOL', 'UC-43'],
            [618, 'IMSG (I-X) BQB', 'girls', 'I-X', 'TARNOL', 'UC-43'],
            [619, 'IMSB (I-VIII) Dhoke Jouri', 'boys', 'I-VIII', 'TARNOL', 'UC-45'],
            [620, 'IMSB (I-VIII) Dhoke Paracha', 'boys', 'I-VIII', 'TARNOL', 'UC-45'],
            [621, 'IMSB (I-X) Maira Beri(F.A)', 'boys', 'I-X', 'TARNOL', 'UC-45'],
            [622, 'IMSB (I-VIII) Chellow', 'boys', 'I-VIII', 'TARNOL', 'UC-50'],
            [623, 'IMSG (I-VIII) Dhoke Jouri', 'girls', 'I-VIII', 'TARNOL', 'UC-45'],
            [624, 'IMSG (I-VIII) Pind Paracha', 'girls', 'I-VIII', 'TARNOL', 'UC-45'],
            [625, 'IMSG (I-VIII) Noon', 'girls', 'I-VIII', 'TARNOL', 'UC-45'],
            [626, 'IMSG (I-VIII) Dhreak Mohri', 'girls', 'I-VIII', 'TARNOL', 'UC-45'],
            [627, 'IMSG (I-X) Maira Beri', 'girls', 'I-X', 'TARNOL', 'UC-45'],
            [628, 'IMSG (I-VIII) Dhoke Paracha', 'girls', 'I-VIII', 'TARNOL', 'UC-45'],
            [629, 'IMSG (I-VIII) Sarae Kharboza (F.A)', 'girls', 'I-VIII', 'TARNOL', 'UC-48'],
            [630, 'IMSB (I-V) Tarnaul', 'boys', 'I-V', 'TARNOL', 'UC-47'],
            [631, 'IMSB (I-V) Tamman', 'boys', 'I-V', 'TARNOL', 'UC-47'],
            [632, 'IMSB (I-V) Sang Jani', 'boys', 'I-V', 'TARNOL', 'UC-45'],
            [633, 'IMSB (I-V) Dora', 'boys', 'I-V', 'TARNOL', 'UC-47'],
            [634, 'IMSB (I-V) Sheikhpur', 'boys', 'I-V', 'TARNOL', 'UC-47'],
            [635, 'IMSB (I-V) Pind Hoon', 'boys', 'I-V', 'TARNOL', 'UC-47'],
            [636, 'IMSB (I-V) Noon', 'boys', 'I-V', 'TARNOL', 'UC-45'],
            [637, 'IMSB (I-V) Shah Allah Ditta', 'boys', 'I-V', 'TARNOL', 'UC-49'],
            [638, 'IMSB (I-V) Karamabad', 'boys', 'I-V', 'TARNOL', 'UC-50'],
            [639, 'IMSB (I-V) Dhoke Lubana', 'boys', 'I-V', 'TARNOL', 'UC-50'],
            [640, 'IMSB (I-V) JOHD', 'boys', 'I-V', 'TARNOL', 'UC-43'],
            [641, 'IMSB (I-V) Sarae Karboza', 'boys', 'I-V', 'TARNOL', 'UC-48'],
            [642, 'IMSB (I-V) Pind Parian', 'boys', 'I-V', 'TARNOL', 'UC-48'],
            [643, 'IMSB (I-V) Dhreak Mohri', 'boys', 'I-V', 'TARNOL', 'UC-45'],
            [644, 'IMSB (I-V) Seri Saral', 'boys', 'I-V', 'TARNOL', 'UC-45'],
            [645, 'IMSB (I-V) Bokra', 'boys', 'I-V', 'TARNOL', 'UC-44'],
            [646, 'IMSB (I-V) Jhangi Syedan (FA)', 'boys', 'I-V', 'TARNOL', 'UC-45'],
            [647, 'IMSB (I-V) Golra', 'boys', 'I-V', 'TARNOL', 'UC-50'],
            [648, 'IMSG (I-V) Bheka Syedan', 'girls', 'I-V', 'TARNOL', 'UC-45'],
            [649, 'IMSG (I-V) Pind Parian', 'girls', 'I-V', 'TARNOL', 'UC-48'],
            [650, 'IMSG (I-V) Sheikhpur', 'girls', 'I-V', 'TARNOL', 'UC-47'],
            [651, 'IMSG (I-V) Dhoke Hashoo', 'girls', 'I-V', 'TARNOL', 'UC-49'],
            [652, 'IMSG (I-V) Dhoke Suleman', 'girls', 'I-V', 'TARNOL', 'UC-49'],
            [653, 'IMSG (I-V) Sarae Madhu', 'girls', 'I-V', 'TARNOL', 'UC-49'],
            [654, 'IMSG (I-V) I-14/3', 'girls', 'I-V', 'TARNOL', 'UC-44'],
            [655, 'IMS (I-VIII) D-17', 'co_education', 'I-VIII', 'TARNOL', 'UC-39'],

            // ═══════════════════════════════════════════════════
            // NILORE (46 schools, EMIS 701-767)
            // Nilore uses alternate UC numbering - mapped to actual DB UC codes
            // ═══════════════════════════════════════════════════
            ...
            [712, 'IMCG,PUNJGRAN', 'girls', 'VI-XII', 'NILORE', 'UC-20'],   // Nilore UC-23 Alipur = main UC-20
            ...
            [716, 'IMCG,THANDA PANI (FA)', 'girls', 'I-XII', 'NILORE', 'UC-20'],  // Nilore UC-20 Alipur
            ...
            [720, 'IMSB(I-X) THANDA PANI', 'boys', 'I-X', 'NILORE', 'UC-52'],  // Thanda Pani UC
            [721, 'IMSB(I-VIII) PEHOUNT', 'boys', 'I-VIII', 'NILORE', 'UC-21'],
            [722, 'IMSG(I-VIII) KH. DAK', 'girls', 'I-VIII', 'NILORE', 'UC-18'],
            [723, 'IMSG(I-X) NEW SHAKRIAL', 'girls', 'I-X', 'NILORE', 'UC-53'],  // Nilore UC-21 New Shakrial = UC-53
            [724, 'IMSG (I-VIII) KALIA (FA)', 'girls', 'I-VIII', 'NILORE', 'UC-09'],  // Nilore UC-24 Chirrah = main UC-09
            [725, 'IMSG(I-X) JABA TAILI', 'girls', 'I-X', 'NILORE', 'UC-20'],
            [726, 'IMSB(I-V)SOHAN', 'boys', 'I-V', 'NILORE', 'UC-21'],
            [727, 'IMSB(I-V)SHARIFABAD', 'boys', 'I-V', 'NILORE', 'UC-19'],  // Nilore UC-47 Tarlai = main UC-19
            [728, 'IMSB(I-V)KHADRAPPAR', 'boys', 'I-V', 'NILORE', 'UC-19'],  // Nilore UC-47 Tarlai = main UC-19
            [729, 'IMSB(I-V) CH. BANGIAL', 'boys', 'I-V', 'NILORE', 'UC-10'],  // Nilore UC-25 Kirpa = main UC-10
            [730, 'IMSB(I-V)CHIRAH', 'boys', 'I-V', 'NILORE', 'UC-09'],
            [731, 'IMSB(I-VIII) KIJNAH', 'boys', 'I-V', 'NILORE', 'UC-08'],  // Nilore UC-21 Tumair = main UC-08
            [732, 'IMSB(I-V) MOHARA SOLINA', 'boys', 'I-V', 'NILORE', 'UC-09'],  // Nilore UC-24 Chirrah = main UC-09
            [733, 'IMSB(I-V) JHANG SYDEN', 'boys', 'I-V', 'NILORE', 'UC-45'],
            [734, 'IMSB(I-V)TARLAI', 'boys', 'I-V', 'NILORE', 'UC-19'],
            [735, 'IMSB(I-V)MOHARA', 'boys', 'I-V', 'NILORE', 'UC-08'],  // Nilore UC-21 Tumair = main UC-08
            [736, 'IMSB(I-V)ARA', 'boys', 'I-V', 'NILORE', 'UC-10'],  // Nilore UC-25 Kirpa = main UC-10
            [737, 'IMSB(I-V)KHANNA KAK', 'boys', 'I-V', 'NILORE', 'UC-54'],  // Dhoke Kala Khan RWP
            [738, 'IMSB(I-V)PINDMISTRIAN', 'boys', 'I-V', 'NILORE', 'UC-52'],  // Nilore UC-22 Thanda Pani = UC-52
            [739, 'IMSG(I-V)SHAKRIAL', 'girls', 'I-V', 'NILORE', 'UC-55'],  // Khana Dak II RWP
            ...
            [743, 'IMSG(I-V)TAMMA', 'girls', 'I-V', 'NILORE', 'UC-19'],  // Nilore UC-47 Tarlai = main UC-19
            [744, 'IMSG(I-V) ALI PUR (MV)', 'girls', 'I-V', 'NILORE', 'UC-20'],
            [745, 'IMSG(I-V)ALI PUR FRASH', 'girls', 'I-V', 'NILORE', 'UC-20'],
            [746, 'IMSG(I-V) SEVERA', 'girls', 'I-V', 'NILORE', 'UC-07'],  // Nilore UC-20 Pind Begwal = main UC-07
            [747, 'IMSG (I-V) CHOUNIAL BANGIAL', 'girls', 'I-V', 'NILORE', 'UC-10'],  // Nilore UC-25 Kirpa = main UC-10
            [748, 'IMSG(I-V)HERNO', 'girls', 'I-V', 'NILORE', 'UC-52'],  // Nilore UC-22 Thanda Pani = UC-52
            [749, 'IMSG(I-X) DARKALA', 'girls', 'I-X', 'NILORE', 'UC-52'],  // Nilore UC-22 Thanda Pani = UC-52
            [750, 'IMSG(I-V) DHOK FATHALL', 'girls', 'I-V', 'NILORE', 'UC-08'],  // Nilore UC-21 Tumair = main UC-08
            [751, 'IMSG(I-V)TUMIAR', 'girls', 'I-V', 'NILORE', 'UC-08'],
            [752, 'IMSG(I-VIII) KIJNAH', 'girls', 'I-VIII', 'NILORE', 'UC-08'],  // Nilore UC-21 Tumair = main UC-08
            [753, 'IMSG(I-V) SIMLY DAM', 'girls', 'I-V', 'NILORE', 'UC-08'],  // Nilore UC-21 Tumair = main UC-08
            [754, 'IMSG(I-V) CHIRAH', 'girls', 'I-V', 'NILORE', 'UC-09'],
            [755, 'IMSG(I-V) KALIA', 'girls', 'I-V', 'NILORE', 'UC-09'],  // Nilore UC-24 Chirrah = main UC-09
            [756, 'IMSG(I-V) JHANG SYEDAN', 'girls', 'I-V', 'NILORE', 'UC-45'],
            [757, 'IMSG (I-V) CHAPPAR Ghasota (F.A)', 'girls', 'I-V', 'NILORE', 'UC-09'],  // Nilore UC-24 Chirrah = main UC-09
            [758, 'IMSG(I-V) CHAKHTAN', 'girls', 'I-V', 'NILORE', 'UC-09'],  // Nilore UC-24 Chirrah = main UC-09
            [759, 'IMSG(I-V) NILORE', 'girls', 'I-V', 'NILORE', 'UC-18'],
            [760, 'IMSG(I-V) PUNJGRAN (760)', 'girls', 'I-V', 'NILORE', 'UC-20'],  // Nilore UC-23 Alipur = main UC-20
            [761, 'IMSG(I-VIII) SOHAN', 'girls', 'I-X', 'NILORE', 'UC-21'],  // User data says I-X level
            [762, 'IMSB(I-V)NILORE', 'boys', 'I-V', 'NILORE', 'UC-18'],
            [763, 'IMSG(I-V) ALI PUR SOUTH', 'girls', 'I-V', 'NILORE', 'UC-20'],
            [764, 'IMSB(I-V) SIRRI', 'boys', 'I-V', 'NILORE', 'UC-52'],  // Nilore UC-22 Thanda Pani = UC-52
            [765, 'IMSG(I-V) FRASH TOWN', 'girls', 'I-V', 'NILORE', 'UC-20'],  // Nilore UC-23 Alipur = main UC-20
            [766, 'IMCG,KIRPA', 'girls', 'VI-XII', 'NILORE', 'UC-10'],
            [767, 'IMSB(I-V)BIATH', 'boys', 'I-V', 'NILORE', 'UC-09'],  // Nilore UC-24 Chirrah = main UC-09

            // ═══════════════════════════════════════════════════
            // MODEL COLLEGES & POST-GRADUATE (EMIS 801-926)
            // ═══════════════════════════════════════════════════
            ...
            [811, 'IMCG I-14/3', 'girls', 'XI-XII', 'TARNOL', 'UC-44'],
            [812, 'IMCG (PG) Bharakahu Islamabad', 'girls', 'XI-XII', 'B-K', 'UC-04'],
            [813, 'HE&MC F-11/1', 'girls', 'XI-XIV', 'URBAN-II', 'UC-29'],
            [901, 'IMCB, F-10/3', 'boys', 'I-XII', 'URBAN-II', 'UC-29'],
            [902, 'IMCB, F-11/1', 'boys', 'I-XII', 'URBAN-II', 'UC-29'],
            [903, 'IMCG, F-11/3', 'girls', 'I-XII', 'URBAN-II', 'UC-29'],
            [904, 'IMCB, F-7/3', 'boys', 'I-XII', 'URBAN-I', 'UC-28'],
            [905, 'IMCB, F-8/4', 'boys', 'I-XII', 'URBAN-I', 'UC-28'],
            [906, 'IMCB, G-10/4', 'boys', 'I-XII', 'URBAN-II', 'UC-36'],
            [907, 'IMCB, G-11/1', 'boys', 'I-XII', 'URBAN-II', 'UC-38'],
            [908, 'ICB G-6/3', 'boys', 'I-XII', 'URBAN-I', 'UC-27'],
            [909, 'IMCB, I-10/1', 'boys', 'I-XII', 'URBAN-II', 'UC-42'],
            [910, 'IMCB, I-8/3', 'boys', 'I-XII', 'URBAN-II', 'UC-40'],
            [911, 'IMCCG (COM), F-10/3', 'girls', 'XI-XIV', 'URBAN-II', 'UC-29'],
            [912, 'IMCG, ST. 25, F-6/2', 'girls', 'I-XII', 'URBAN-I', 'UC-25'],
            [913, 'ICG, F-6/2', 'girls', 'I-XII', 'URBAN-I', 'UC-25'],
            [914, 'IMCG, F-7/4', 'girls', 'I-XII', 'URBAN-I', 'UC-28'],
            [915, 'IMCG, F-8/1', 'girls', 'I-XII', 'URBAN-I', 'UC-28'],
            [916, 'IMCG, G-10/2', 'girls', 'I-XII', 'URBAN-II', 'UC-37'],
            [917, 'IMCG, St. # 23, I-10/4', 'girls', 'I-XII', 'URBAN-II', 'UC-42'],
            [918, 'IMCG, I-8/4', 'girls', 'I-XII', 'URBAN-II', 'UC-40'],
            [919, 'IMCG, F-10/2', 'girls', 'I-XII', 'URBAN-II', 'UC-29'],
            [920, 'IMCG, Korang Town', 'girls', 'I-XII', 'SIHALA', 'UC-17'],
            [921, 'IMCG G-13/1', 'girls', 'I-XII', 'TARNOL', 'UC-39'],
            [922, 'IMCG G-14/4', 'girls', 'I-XII', 'TARNOL', 'UC-39'],
            [923, 'IMCB G-13/2', 'boys', 'I-XII', 'TARNOL', 'UC-39'],
            [924, 'IMCB G-15', 'boys', 'I-XII', 'TARNOL', 'UC-39'],
            [925, 'IMCB Pakistan Town', 'boys', 'I-XII', 'SIHALA', 'UC-17'],
            [926, 'IMCB Maira Begwal', 'boys', 'I-XII', 'B-K', 'UC-07'],
        ];

        $count = 0;
        foreach ($schools as $s) {
            $sectorId = $sectors[$s[4]] ?? null;
            $ucId     = $ucs[$s[5]] ?? null;

            if (!$sectorId) {
                $this->command->error("Sector not found: {$s[4]} for EMIS {$s[0]}");
                continue;
            }
            if (!$ucId) {
                $this->command->error("UC not found: {$s[5]} for EMIS {$s[0]}");
                continue;
            }
        ...
        $this->command->info("Done! {$count} schools inserted.");

        // Verify counts
        $total = Institution::count();
        $bySector = Institution::select('sector_id', DB::raw('count(*) as cnt'))
            ->groupBy('sector_id')
            ->get();

        $this->command->info("Total institutions: {$total}");
        foreach ($bySector as $row) {
            $sectorName = Sector::find($row->sector_id)?->name ?? 'Unknown';
            $this->command->info("  {$sectorName}: {$row->cnt}");
        }

        if ($total !== 432) {
            $this->command->error("WARNING: Expected 432 schools but got {$total}!");
        }
    }
}

{{-- SAVE AS: resources/views/director/monitoring/index.blade.php --}}
@extends('layouts.app')
@section('title', 'Admission Monitoring — Director View')
...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">
                Real-time system-wide view — all institutions
                @if ($academicYear)
    ...
    <form method="GET" class="flex flex-wrap gap-3 mb-5">
        ...
        <select name="test_status"
            ...
            <option value="not_applicable" {{ request('test_status') === 'not_applicable' ? 'selected' : '' }}>N/A</option>
            <option value="scheduled" {{ request('test_status') === 'scheduled' ? 'selected' : '' }}>Scheduled
            </option>
            <option value="conducted" {{ request('test_status') === 'conducted' ? 'selected' : '' }}>Conducted
            </option>
        ...
        <select name="doc_status"
            ...
            <option value="affidavit" {{ request('doc_status') === 'affidavit' ? 'selected' : '' }}>Affidavit</option>
        ...
        @if (request()->hasAny(['sector_id', 'institution_id', 'test_status', 'merit_status', 'doc_status']))
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="border-b-2 border-gray-100 bg-gray-50">
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase hidden md:table-cell">Sector</th>
                ...
                <tbody>
                    @forelse($records as $rec)
                        @php
                            $testBadge = match ($rec->test_status) {
                        ...
                        <tr class="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-3 max-w-[160px]">
                                <p class="font-medium text-gray-800 text-sm truncate max-w-[160px]" title="{{ $rec->institution->name }}">{{ $rec->institution->name }}</p>
                                <p class="text-xs text-gray-400">{{ $rec->institution->code }}</p>
                            </td>
                            <td class="px-4 py-3 text-xs text-gray-500 hidden md:table-cell">
                                {{ $rec->institution->sector?->name ?? '—' }}
                            </td>

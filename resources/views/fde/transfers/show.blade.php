{{-- SAVE AS: resources/views/fde/transfers/show.blade.php --}}
@extends('layouts.app')
@section('title', 'Transfer #' . $transfer->id . ' — FDE Cell')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                @if ($transfer->isCrossSector())
                    <div class="bg-orange-50 border border-orange-200 rounded-xl px-4 py-3 mb-4 text-xs text-orange-800">
                        ⚠️ This is a <strong>cross-sector transfer</strong> — the sending and receiving schools belong to
                        different sectors.
                        <span class="block mt-0.5 text-orange-600">
                            From: <strong>{{ $transfer->fromInstitution->sector?->name ?? '—' }}</strong>
                            &nbsp;→&nbsp;
                            To: <strong>{{ $transfer->toInstitution->sector?->name ?? '—' }}</strong>
                        </span>
                    </div>
                @endif
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400">{{ $transfer->created_at->format('d M Y, g:i A') }} by
                                ...
                                ({{ $transfer->initiated_by_role === 'fde_cell' ? 'FDE Cell' : 'HOI' }})</p>
                        </div>
                    </div>
                    @if ($transfer->isCrossSector())
                        @if ($transfer->isCrossSectorApproved())
                            <div class="flex items-start gap-3">
                                <span class="w-2 h-2 rounded-full bg-orange-500 mt-1.5 shrink-0"></span>
                                <div>
                                    <p class="font-medium text-orange-700">⚡ Cross-sector approved</p>
                                    <p class="text-xs text-gray-400">
                                        {{ $transfer->cross_sector_approved_at?->format('d M Y, g:i A') }}
                                        by {{ $transfer->crossSectorApprovedBy?->name }}</p>
                                    @if ($transfer->cross_sector_note)
                                        <p class="text-xs text-orange-700 mt-1 bg-orange-50 rounded px-2 py-1">
                                            {{ $transfer->cross_sector_note }}</p>
                                    @endif
                                </div>
                            </div>
                        @else
                            <div class="flex items-start gap-3">
                                <span class="w-2 h-2 rounded-full bg-orange-300 mt-1.5 shrink-0"></span>
                                <div>
                                    <p class="font-medium text-orange-600">⏳ Awaiting cross-sector approval</p>
                                    <p class="text-xs text-gray-400">FDE sign-off required before this transfer can proceed.
                                    </p>
                                </div>
                            </div>
                        @endif
                    ...
                    @if ($transfer->isInfoRequested())
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-400">
                                    {{ $transfer->info_requested_at?->format('d M Y, g:i A') }}</p>
                    ...
                    @if ($transfer->isAccepted())
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mt-1">Enrollment counts updated — sending school -1,
                                    receiving school +1.</p>
                            </div>
                        </div>
                    @endif
                    ...
                    @if ($transfer->isCancelled())
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <p class="font-medium text-gray-600">Cancelled</p>
        ...
        <div class="space-y-4">
            ...
            @if ($transfer->isActionable())
                ...
                {{-- ⚡ Cross-Sector Approval (shown first if needed) --}}
                @if ($transfer->needsCrossSectorApproval())
                    <div class="bg-white rounded-xl shadow-sm border-2 border-orange-300 p-5">
                        <h4 class="text-sm font-bold text-orange-800 mb-1">⚡ Cross-Sector Approval Required</h4>
                        <p class="text-xs text-gray-500 mb-3">
                            This transfer moves a student between sectors
                            (<strong>{{ $transfer->fromInstitution->sector?->name ?? '—' }}</strong>
                            → <strong>{{ $transfer->toInstitution->sector?->name ?? '—' }}</strong>).
                            FDE sign-off is required before proceeding.
                        </p>
                        <form method="POST" action="{{ route('fde.transfers.cross-sector', $transfer) }}">
                            @csrf
                            <textarea name="cross_sector_note" rows="3" required minlength="10"
                                placeholder="Reason / justification for cross-sector approval..."
                                class="w-full border border-orange-200 rounded-lg px-3 py-2 text-sm
                                       focus:outline-none focus:ring-2 focus:ring-orange-400 mb-3"></textarea>
                            <button type="submit" onclick="return confirm('Approve this cross-sector transfer?')"
                                class="w-full py-2.5 bg-orange-500 text-white text-sm font-bold rounded-lg hover:bg-orange-600 transition">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-green-100 p-5">
                    <h4 class="text-sm font-bold text-gray-800 mb-1">✅ Force Accept</h4>
                    <p class="text-xs text-gray-500 mb-3">Override and accept regardless of receiving HOI's response.
                        Enrollment will update immediately.</p>
                    <form method="POST" action="{{ route('fde.transfers.accept', $transfer) }}"
                        onsubmit="return confirm('Force-accept this transfer? Enrollment counts will update immediately.')">
                        @csrf
                        <button type="submit"
                ...
                <div class="bg-white rounded-xl shadow-sm border border-red-100 p-5">
                    <h4 class="text-sm font-bold text-gray-800 mb-2">❌ Reject</h4>
                    <form method="POST" action="{{ route('fde.transfers.reject', $transfer) }}">
                        @csrf
                        ...
                        <button type="submit" onclick="return confirm('Reject this transfer?')"
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                    <h4 class="text-sm font-bold text-gray-800 mb-2">🚫 Cancel</h4>
                    <form method="POST" action="{{ route('fde.transfers.cancel', $transfer) }}">
                        @csrf
                        <textarea name="cancellation_reason" rows="2" placeholder="Reason (optional)..."

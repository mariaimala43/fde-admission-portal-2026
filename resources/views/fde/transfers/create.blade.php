{{-- SAVE AS: resources/views/fde/transfers/create.blade.php --}}
@extends('layouts.app')
@section('title', 'Initiate Student Transfer')

@section('content')

    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Initiate Student Transfer</h2>
            <p class="text-sm text-gray-500 mt-1">FDE Cell — transfer between any two schools</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="bg-blue-50 border border-blue-100 rounded-lg px-4 py-3 mb-6 text-sm text-blue-700">
            ℹ️ Each student row becomes a <strong>separate transfer request</strong>.
            The sending and receiving school are shared across all rows.
            Each student transfers into the <strong>same class</strong> at the receiving school.
        </div>

        <form method="POST" action="{{ route('fde.transfers.store') }}">
            @csrf

            {{-- From / To Schools — shared for all rows --}}
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">
                        Sending School <span class="text-red-500">*</span>
                    </label>
                    <select name="from_institution_id" required
                        class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="">— Select sending school —</option>
                        @foreach ($institutions as $inst)
                            <option value="{{ $inst->id }}"
                                {{ old('from_institution_id') == $inst->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="to_institution_id" required
                        ...
                        @foreach ($institutions as $inst)
                            <option value="{{ $inst->id }}"
                                {{ old('to_institution_id') == $inst->id ? 'selected' : '' }}>
                                {{ $inst->name }}{{ $inst->code ? " ($inst->code)" : '' }}
                            </option>
                        @endforeach
                    </select>
    ...
    <script>
        const classOptions = @json($classes->map(fn($c) => ['id' => $c->id, 'name' => $c->name]));
    </script>

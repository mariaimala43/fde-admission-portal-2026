...
@section('content')
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('fde.export.master') }}"
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        ...
        <div
            ...
            <p class="text-xs text-gray-400 uppercase tracking-wider mb-1">Existing Enroll.</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
        ...
        {{-- Category Breakdown by Sector (stacked) --}}
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
            <h3 class="text-sm font-bold text-gray-800 dark:text-gray-100 mb-1">Category Breakdown by Sector</h3>
            <p class="text-xs text-gray-400 mb-4">Regular vs OOSC vs P2P stacked</p>
            <div class="relative h-64">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
        ...
        {{-- OOSC + P2P by Sector --}}
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
            <h3 class="text-sm font-bold text-gray-800 dark:text-gray-100 mb-1">OOSC &amp; P2P by Sector</h3>
    ...
    {{-- ── School Fill Rates Table ───────────────────────────────────────────── --}}
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
            <div>
                <h3 class="text-sm font-bold text-gray-800 dark:text-gray-100">School Fill Rates</h3>
                <p class="text-xs text-gray-400 mt-0.5">Sorted by fill rate — highest first</p>
            </div>
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 dark:bg-gray-900 text-xs uppercase text-gray-400">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left min-w-[120px]">Fill Rate</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($schoolFillRates->take(20) as $school)
                        <tr class="border-b border-gray-50 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-750">
                            <td class="px-4 py-3 font-medium text-gray-800 dark:text-gray-200">{{ $school->name }}</td>
                            <td class="px-4 py-3 text-xs text-gray-500">{{ $school->sector?->name }}</td>
                            <td class="px-4 py-3 text-center text-gray-600 dark:text-gray-300">
                                {{ number_format($school->seats) }}</td>
                            <td class="px-4 py-3 text-center font-semibold text-blue-700 dark:text-blue-300">
                                {{ number_format($school->admitted) }}</td>
                            <td
                                class="px-4 py-3 text-center {{ $school->remaining > 0 ? 'text-green-600' : 'text-red-500' }}">
                                {{ number_format($school->remaining) }}
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <div class="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full h-2">
                                        <div class="h-2 rounded-full {{ $school->fill_rate >= 90 ? 'bg-green-500' : ($school->fill_rate >= 60 ? 'bg-blue-500' : 'bg-orange-400') }}"
                                            style="width: {{ min(100, $school->fill_rate) }}%"></div>
                                    </div>
                                    <span
                                        class="text-xs font-bold text-gray-600 dark:text-gray-300 w-10 text-right">{{ $school->fill_rate }}%</span>
                                </div>
                            </td>
                        </tr>
                    @empty
    ...
    <script>
        ...
        const catRegular = {{ $categoryData['regular'] ?? 0 }};
        const catOosc = {{ $categoryData['oosc'] ?? 0 }};
        const catP2p = {{ $categoryData['p2p'] ?? 0 }};
        ...
        new Chart(document.getElementById('genderChart'), {
            type: 'doughnut',
            data: {
                labels: ['Reg Boys', 'Reg Girls', 'OOSC Boys', 'OOSC Girls', 'P2P Boys', 'P2P Girls'],
                datasets: [{
                    data: [
                        genderData.reg_boys,
                        genderData.reg_girls,
                        genderData.oosc_boys,
                        genderData.oosc_girls,
                        genderData.p2p_boys,
                        genderData.p2p_girls,
                    ],

        ...
        <table class="w-full text-sm">
            ...
            <tfoot class="bg-blue-50 border-t-2 border-blue-100 font-bold text-sm">
                <tr>
                    ...
                    <td class="px-4 py-3 text-center text-blue-900 bg-blue-100">{{ number_format($grand['enrollment']) }}
                    </td>
                </tr>
            </tfoot>
        </table>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    @foreach ($sectorInsts as $inst)
                        @php
                            $instClasses = $seatData->get($inst->id, collect());
                            $instAdm = $admissionData->get($inst->id, collect());
                            $instSecs = $sectionCounts->get($inst->id, collect());
                        ...
                        <tbody x-data="{ open: false }">
                            ...
                            @foreach ($instClasses->sortBy('class_id') as $ic)
                                @php
                                    $adm = $instAdm->get($ic->class_id);
                                    $admitted = $adm?->total_admitted ?? 0;
                                    $secCount = $instSecs->get($ic->class_id)?->section_count ?? 0;
                                ...
                                <tr x-show="open" class="border-b border-gray-50 hover:bg-blue-50 bg-white">
                                    ...
                                    <td class="px-4 py-2.5 text-center text-orange-600">
